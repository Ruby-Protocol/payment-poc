...
contract RubyRedeem {
    ...
    function verify(Statement memory statement, Proof memory proof) public view returns (bool) {
        ...
        sigma_info.gEpoch = Utils.mapInto("Ruby", statement.epoch);
        ...
        require(sigma_info.c == proof.c, string(abi.encodePacked("[Ruby redeem] Sigma challenge failed: ", Utils.uint2str(statement.epoch))));
        ...
        require(ip.verifyInnerProduct(ip_info.hPrimes, ip_info.u_x, ip_info.P, proof.ip_proof, ip_info.o), "[Ruby redeem] Inner product proof verification failed.");
